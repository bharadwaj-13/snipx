...
export default function EditSnippet() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!data || data.user_id !== user.id) { navigate('/dashboard'); return }
      setTitle(data.title)
  ...
  if (fetching) return (
    <div style={{ minHeight: '100vh', background: '#0d1117' }}>
      <Navbar />
      <div style={{ color: '#484f58', fontFamily: 'monospace', padding: '3rem 2rem' }}>loading...</div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d1117' }}>
      ...
      <div style={{ maxWidth: '780px', margin: '0 auto', padding: '2rem 1.5rem' }}>
        <h2 style={{ color: '#e6edf3', fontWeight: '500', marginBottom: '1.5rem' }}>edit snippet</h2>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
            <button type="button" onClick={() => navigate(`/snippet/${id}`)} style={{

...
import { getSnippetById, updateSnippet } from '../services/snippets'
import { createCollection, getCollections } from '../services/collections'
import { LuCopy, LuSave, LuCode, LuX, LuPlus, LuTags, LuCheck, LuArrowLeft } from 'react-icons/lu'
...
export default function EditSnippet() {
  ...
  useEffect(() => {
    async function load() {
      const [{ data }, { data: cols }] = await Promise.all([
        getSnippetById(id), getCollections(user.id),
      ])
      if (!data || data.user_id !== user.id) { navigate(-1); return }
  ...
  if (fetching) return <div className="page-content mono" style={{ color: 'var(--text-muted)' }}>Initializing Console...</div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <button onClick={handleSave} disabled={loading || !title || !code} className="btn btn-primary" style={{ padding: '6px 16px', fontSize: '12px' }}>
              <LuSave size={14} style={{ marginRight: '6px' }} /> {loading ? 'Saving...' : 'Save Updates'}
            </button>
          </div>
        </div>
